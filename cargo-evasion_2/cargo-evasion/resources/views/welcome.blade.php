    ...
    <section class="pt-32 md:pt-48 pb-20 px-6 overflow-hidden">
        <div class="max-w-7xl mx-auto grid lg:grid-cols-2 gap-12 lg:gap-24 items-center">
            ...
            <div class="relative mt-12 lg:mt-0">
                ...
                <div class="milly-img-frame" x-data="heroSlider({{ $availableBikes->count() }})">
                    @forelse($availableBikes as $index => $bike)
                        <div x-show="active === {{ $index }}"
                             x-transition:enter="milly-hero-fade-enter"
                             x-transition:enter-end="milly-hero-fade-enter-active"
                             x-transition:leave="milly-hero-fade-exit"
                             x-transition:leave-end="milly-hero-fade-exit-active"
                             class="absolute inset-0">
                            
                            <img src="{{ asset('storage/' . $bike->image) }}" 
                                 alt="{{ $bike->model }}" 
                                 class="milly-img-cover opacity-80">
                        </div>
                    @empty
